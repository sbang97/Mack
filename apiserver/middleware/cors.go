...
func CORS(origins, methods, allowHeaders, exposeHeaders string) Adapter {
	...
	if len(origins) == 0 {
		origins = headerAccessControlAllowOrigin
	}
	if len(methods) == 0 {
		methods = headerAccessControlAllowMethods
	}
	if len(allowHeaders) == 0 {
		allowHeaders = headerAccessControlAllowHeaders
	}
	if len(exposeHeaders) == 0 {
		exposeHeaders = headerAccessControlExposeHeaders
	}
	...
	return func(handler http.Handler) http.Handler {
		...
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			w.Header().Set(origins, DefaultCORSOrigins)
			w.Header().Set(methods, DefaultCORSMethods)
			w.Header().Set(allowHeaders, DefaultCORSAllowHeaders)
			w.Header().Set(exposeHeaders, DefaultCORSExposeHeaders)
